...
export function useMapHandlers(props: UseMapHandlersProps): UseMapHandlersReturn {
  ...
  const handleAnalyzeArea = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || 'Failed to analyze area')
      }

      // Store the AI analysis result in sessionStorage for AICopilot to pick up
