...
export function useMapInitialization(props: UseMapInitializationProps): UseMapInitializationReturn {
  ...
  // Handle draw created for AI analysis mode
  useEffect(() => {
    const map = mapRef.current
    if (!map) return
    ...
    const handleDrawCreated = (event: any) => {
      if (aiAnalysisMode) {
        ...
        setAiAnalysisPolygon(polygon)

        // Calculate area and store it
        const areaKm2 = calculatePolygonAreaKm2(polygon)
        setAiAnalysisArea(areaKm2)
      }
    }
  ...
  }, [aiAnalysisMode, setAiAnalysisPolygon, setAiAnalysisArea, setSelectedCountry])

  // Show/hide draw control based on reporting mode or stats mode or AI analysis mode
  useEffect(() => {
    if (!mapRef.current || !drawControlRef.current) return

    const shouldShowDrawControl = reportingMode || statsMode || aiAnalysisMode
    const map = mapRef.current
  ...
  }, [reportingMode, statsMode, aiAnalysisMode])
