...
export default function WaterWatchAfricaPage() {
    ...
    const load = async () => {
      ...
      chartInstance = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            data: [35.83, 36.77, 37.82, 39.04, 41.95, 43.29, 44.57, 45.73, 46.82, 47.85, 48.81],
            borderColor: '#16a34a',
            borderWidth: 2.5,
  ...
  const kpis = [
    { label: 'Countries covered', value: 54, suffix: '', decimals: 0, sub: 'African Union members', accent: false, display: null as string | null },
    { label: 'Continental avg, 2020', value: 47.4, suffix: '%', decimals: 1, sub: 'safely managed access', accent: true, display: null as string | null },
    { label: 'Above 50% threshold', value: null as number | null, suffix: '', decimals: 0, sub: 'countries w/ available data', accent: false, display: '17/34' },
    { label: '20-year improvement', value: 13, suffix: ' pp', decimals: 0, sub: 'avg gain across continent', accent: false, display: null as string | null },
  ]
    ...
    <div style={{ minHeight: '100vh', background: '#f3f4f6', fontFamily: "'Instrument Sans', sans-serif", color: '#111827' }}>
      ...
      <header style={{ position: 'relative', overflow: 'hidden', padding: '64px 64px 56px', background: '#ffffff', borderBottom: '1px solid #e5e7eb' }}>
        ...
        <Stagger delay={0}>
          <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '10px', letterSpacing: '0.2em', color: '#16a34a', marginBottom: '20px', textTransform: 'uppercase' }}>
            ◈ PRAIS4 Consolidated Dataset · Africa · 2000–2020
          </p>
        </Stagger>
        <Stagger delay={70}>
        ...
        <Stagger delay={140}>
          <h1 style={{ fontFamily: "'Clash Display', sans-serif", fontSize: 'clamp(2.8rem,5.5vw,5rem)', lineHeight: 0.92, letterSpacing: '-0.03em', fontWeight: 700, marginBottom: '40px' }}>
            <em style={{ color: '#16a34a', fontStyle: 'italic' }}>Across 54 Nations</em>
          </h1>
        </Stagger>
        <Stagger delay={220}>
      ...
      <main style={{ maxWidth: '1280px', margin: '0 auto', padding: '56px 64px 80px' }}>
          ...
          {kpis.map((kpi, i) => (
            <Stagger key={i} delay={i * 55} style={{ height: '100%' }}>
              <div style={{ background: '#ffffff', padding: '28px', position: 'relative', overflow: 'hidden', height: '100%' }}>
                {kpi.accent && (
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '3px', background: 'linear-gradient(90deg,#16a34a,transparent)' }} />
                )}
                <p style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '10px', letterSpacing: '0.14em', color: '#9ca3af', textTransform: 'uppercase', marginBottom: '14px' }}>
                  {kpi.label}
                </p>
                <p style={{ fontFamily: "'Clash Display', sans-serif", fontSize: '3rem', lineHeight: 1, fontWeight: 600, marginBottom: '8px', color: kpi.accent ? '#16a34a' : '#0f172a' }}>
                  {kpi.display
                    ? kpi.display
                    : <Counter target={kpi.value as number} suffix={kpi.suffix} decimals={kpi.decimals} />
                  }
                </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '3fr 2fr', gap: '16px', marginBottom: '16px' }}>
          <Stagger delay={0}>
            <div style={card}>
              {sectionHead('#16a34a', 'Continental trend — % safely managed water')}
          ...
          <Stagger delay={80}>
            <div style={card}>
              {sectionHead('#ef4444', 'Lowest access — 2020')}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
                {lowestCountries.map((d, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', color: '#d1d5db', width: '20px', textAlign: 'right', flexShrink: 0 }}>
                      {String(i + 1).padStart(2, '0')}
                    </span>
                    <span style={{ fontSize: '16px', color: '#374151', width: '115px', flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {d.name}
                    </span>
                    <Bar pct={d.val / 30 * 100} color="linear-gradient(90deg,#ef4444,#f97316)" delay={i * 60} />
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '15px', color: '#0f172a', width: '44px', textAlign: 'right', flexShrink: 0 }}>
                      {d.val.toFixed(1)}%
                    </span>
                  </div>
                ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '16px' }}>
          <Stagger delay={0}>
            <div style={card}>
              {sectionHead('#16a34a', 'Biggest gains 2000 → 2020 (pp)')}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '13px' }}>
                {improvers.map((d, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '12px', color: '#d1d5db', width: '20px', textAlign: 'right', flexShrink: 0 }}>
                    ...
                    <span style={{ fontSize: '16px', color: '#374151', width: '125px', flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {d.name}
                    </span>
                    <Bar pct={d.val / 40 * 100} color="linear-gradient(90deg,#16a34a,#0ea5e9)" delay={i * 60} />
                    <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: '15px', color: '#16a34a', width: '44px', textAlign: 'right', flexShrink: 0 }}>
                      +{d.val.toFixed(1)}
                    </span>
                  </div>
                ))}
          ...
          <Stagger delay={80}>
            <div style={card}>
              {sectionHead('#6366f1', 'Notable country changes')}
                  ...
                  <tr>
                    {['Country','2000','2020','Delta'].map((h, i) => (
                      <th key={i} style={{
                        ...
                        padding: '0 0 12px', textAlign: i === 3 ? 'right' : 'left',
                    ...
                    <tr key={i}>
                      ...
                      <td style={{ padding: '10px 0', textAlign: 'right', borderBottom: '1px solid #f9fafb' }}>
                        <span style={{
                          ...
                          color: r.change >= 0 ? '#15803d' : '#b91c1c',
                          border: `1px solid ${r.change >= 0 ? '#bbf7d0' : '#fecaca'}`,
                        }}>
        ...
        <Stagger delay={0}>
          <div style={{ ...card, marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '32px' }}>
              {sectionHead('#0ea5e9', 'Urban vs rural gap — biggest disparities in 2020')}
              <div style={{ display: 'flex', gap: '20px', fontSize: '11px', color: '#9ca3af', marginBottom: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px' }}>
          {[
            { icon: '↗', title: 'Steady continental progress', desc: 'The continental average climbed from 35.8% in 2000 to 48.8% in 2020 — meaningful progress, though the continent as a whole remains just below the 50% mark.', accent: '#16a34a' },
            ...
            { icon: '⊘', title: 'The persistent rural deficit', desc: 'In Gambia, Lesotho, and Eswatini, the urban-rural gap exceeds 50 percentage points. Urban populations are often 3–5× more likely to have safely managed water than rural counterparts.', accent: '#0ea5e9' },
          ].map((item, i) => (
            <Stagger key={i} delay={i * 80}>
              <div style={{ ...card, borderTop: `3px solid ${item.accent}`, height: '100%' }}>
                ...
                <p style={{ fontFamily: "'Clash Display', sans-serif", fontSize: '1.2rem', fontWeight: 600, color: '#0f172a', marginBottom: '10px', lineHeight: 1.3 }}>
                  {item.title}
                </p>
